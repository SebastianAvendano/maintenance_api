...
/* =============================================
Se crea la clase AbstracModel para almacenar los modelos de consulta, creacion y actualizacion de datos
============================================= */
  ...
  async searchByPage (data, page) {
    if (page > 0) {
      let results = await this.model
        ...
        .sort(this.sort)
        .limit(this.perPage).exec()
      if (this.populate) {
        results = await this.model.find(data, this.fields)
          .populate(this.populate).skip((this.perPage * page) - this.perPage) 
